...
test.each([
  ...
  const { t } = await setup();
  expect(await t.query(internal.chat.bot.morningGreetingDue, {})).toBe(
    expected,
  );
});
...
test("posts once per day as Verity in Everyone, with no fake prompt or reply", async () => {
  ...
  expect(await t.mutation(internal.chat.bot.publishMorningGreeting, args)).toBe(
    true,
  );
  expect(await t.mutation(internal.chat.bot.publishMorningGreeting, args)).toBe(
    false,
  ...
  expect(
    await t.mutation(internal.chat.bot.publishMorningGreeting, {
      ...args,
      day: "2026-09-15",
    }),
  ).toBe(true);
