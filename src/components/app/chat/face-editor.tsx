...
export function FaceEditor({
  ...
  const [customMode, setCustomMode] = useState<"emoji" | "text">(
    face.initials ? "text" : "emoji",
  );
  ...
  return (
    <Popover.Root
      open={open}
      onOpenChange={(next) => {
        if (next) setLetters(face.initials ?? "");
        setOpen(next);
      }}
    >
      ...
      <Popover.Portal>
        ...
        >
          <Popover.Popup className="w-72 max-w-[calc(100vw-2rem)] rounded-2xl border border-border bg-popover p-3 text-popover-foreground shadow-xl">
            <Popover.Title className="mb-2 text-sm font-semibold">
              Picture
            </Popover.Title>
            <OptionTiles
              value={mode}
              onPick={(next: "account" | "emoji" | "text") => {
                if (next === "account") account?.onSelect();
                  ...
                  if (next === "text")
                    setLetters(
                      face.initials ??
                        name.slice(0, 1).replace(/[^a-z0-9]/gi, ""),
                    );
                  onChange({
                    hue: face.hue,
                    ...(next === "text"
                      ...
                      : { emoji: face.emoji ?? AVATAR_EMOJI[0] }),
                  });
                }
              }}
              className={cn(
                "grid gap-1",
                account ? "grid-cols-3" : "grid-cols-2",
              )}
              options={[
                ...(account
            ...
            {mode === "account" ? (
              <p className="my-3 text-xs text-muted-foreground">
                Uses your account profile picture. Change it in account
                settings.
              </p>
            ) : mode === "emoji" ? (
              <div className="my-3 grid grid-cols-8 gap-1">
                {AVATAR_EMOJI.map((emoji) => (
                  <button
                    ...
                    type="button"
                    aria-label={emoji}
                    aria-pressed={face.emoji === emoji}
                    onClick={() => onChange({ hue: face.hue, emoji })}
                    className={cn(
                      "rounded-lg p-1 hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring",
                      face.emoji === emoji && "bg-primary/15",
                    )}
                  >
                    {emoji}
                  </button>
              ...
              <input
                ...
                onChange={(event) => {
                  const initials = event.target.value.replace(
                    /[^a-z0-9]/gi,
                    "",
                  );
                  setLetters(initials);
              ...
              >
                {AVATAR_HUES.map((hue) => (
                  <button
                    ...
                    aria-pressed={face.hue === hue}
                    onClick={() => onChange({ ...face, hue })}
                    style={{ "--monogram-hue": hue } as CSSProperties}
                    className={cn(
                      "monogram size-6 rounded-full border-2 focus-visible:ring-2 focus-visible:ring-ring",
                      face.hue === hue
                        ? "border-primary"
                        : "border-transparent",
                    )}
            ...
            <div className="flex justify-between">
              <Button
                variant="ghost"
                size="sm"
                onClick={() => (account ? account.onSelect() : onChange({}))}
              >
                Reset
              </Button>
