...
const LENGTHS = FINGERS.map((_, i) =>
  Number((83 + (i % 5) * 11 + Math.sin(i * 4) * 13).toFixed(4)),
);
const LOOP_MS = 12000;
const SAMPLES = 240;
...
export function PhilosophyPuppets() {
  ...
  useEffect(() => {
    ...
    root
      .querySelectorAll<HTMLElement>("[data-pendulum]")
      .forEach((pendulum, i) => {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="Two hands gently bobbing, with ten realistic phones swinging from strings attached to their fingers."
    >
      {[0, 1].map((side) => (
        <div
          key={side}
