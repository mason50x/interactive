/**
 * The player origin: the one place activity code is allowed to run.
 *
 * An activity is third-party code. Even one we wrote today is a bundle we will
 ...
 * user. Give activities a different hostname and the browser enforces the rest —
 ...
 * hostname.
 *
 * `null` is the un-isolated fallback: activities are served from `/player/<slug>`
 ...
 * so `ActivityFrame` drops `allow-same-origin` from the sandbox to compensate —
...
/**
 * The `src` for an activity's frame, carrying the grant that buys it past the
 * proxy — see `src/lib/player-token.ts`. Without one the player origin
 * answers 404, so this is the only way to build a working activity URL.
 *
 * On the player origin the `/player` prefix is supplied by the proxy, so the
 * public URL of an activity is just its slug at the root — which is also what
