...
test("Clerk cutover preserves profile identity, history and privacy; repeat sync preserves avatar choice", async () => {
  ...
  expect(
    await t.run((ctx) => profileByHandle(ctx, "real-username")),
  ).toMatchObject({ _id: id });
  expect(await t.run((ctx) => profileByHandle(ctx, "old_handle"))).toBeNull();
  await mine.mutation(api.chat.profiles.setAvatar, {
    mode: "custom",
    initials: "AB",
    hue: 30,
  });
  await t.mutation(internal.users.upsertFromClerk, {
    data: { ...data, username: "NewName", first_name: "New", updated_at: 20 },
  });
  expect(await mine.query(api.chat.profiles.mine, {})).toMatchObject({
    handle: "NewName",
    displayName: "New",
  ...
  expect(
    (await mine.query(api.chat.profiles.mine, {}))?.avatarUrl,
  ).toBeUndefined();
  // Old webhook delivery and old JWT sync cannot overwrite the authoritative update.
  await t.mutation(internal.users.upsertFromClerk, { data });
  await mine.mutation(api.users.store, {});
  expect((await mine.query(api.chat.profiles.mine, {}))?.handle).toBe(
    "NewName",
  );
  await mine.mutation(api.chat.profiles.setAvatar, { mode: "account" });
  expect((await mine.query(api.chat.profiles.mine, {}))?.avatarUrl).toBe(
    data.image_url,
  );
  expect(
    await mine.mutation(api.chat.attachments.uploadUrl, { purpose: "avatar" }),
  ).toEqual({ ok: false, refusal: "image" });
});
...
test("new account receives one profile and global membership; Clerk identities do not use confusable folding", async () => {
  ...
  const rows = await t.run(async (ctx) => ({
    profiles: await ctx.db.query("chatProfiles").take(10),
    members: await ctx.db.query("conversationMembers").take(10),
    one: await profileByHandle(ctx, "alice"),
  ...
  expect(rows.members.filter((member) => member.dmPeer === "bot")).toHaveLength(
    2,
  );
  expect(rows.one?.clerkId).toBe("one");
  expect(rows.two?.clerkId).toBe("two");
...
test("messages keep their ids and show the current Clerk identity after rename", async () => {
  ...
  const room = await t.run(async (ctx) => {
    const profile = await ctx.db
      .query("chatProfiles")
      .withIndex("byClerkId", (q) => q.eq("clerkId", "sender"))
      .unique();
...
test("first names are formatted and collisions use the shortest available surname prefix", async () => {
  ...
  for (const data of accounts)
    await t.mutation(internal.users.upsertFromClerk, { data });
  const names = async () =>
    (await t.run((ctx) => ctx.db.query("chatProfiles").take(20))).map(
      (p) => p.displayName,
    );
