...
test("only server-authorized admins receive two hours, independently of other accounts", async () => {
  ...
  expect(result.allowanceSeconds).toBe(7_200);
  expect(result.remainingSeconds).toBe(7_185);
  const other = await t
...
test.each([
  ...
  const { t, user } = setup(subject);
  await user.mutation(api.experience.acquire, { sessionId: "old" });
  await t.run(async ctx => {
    const row = await ctx.db.query("experienceLeases").first();
    await ctx.db.patch(row!._id, { allowanceSeconds: oldAllowance, until: start });
    const key = `${subject}:${Math.floor(start / DAY)}`;
    await ctx.runMutation(components.rateLimiter.lib.resetRateLimit, { name: "experienceSeconds", key });
    await ctx.runMutation(components.rateLimiter.lib.rateLimit, {
      name: "experienceSeconds", key, count: used,
      config: { kind: "fixed window", rate: oldAllowance, period: DAY, start: Math.floor(start / DAY) * DAY },
    });
  });
  expect((await user.query(api.experience.status, { day: 0 })).remainingSeconds).toBe(remaining);
  await user.mutation(api.experience.release, { sessionId: "old" });
  await user.mutation(api.experience.release, { sessionId: "old" });
  expect((await user.query(api.experience.status, { day: 0 })).remainingSeconds).toBe(remaining);
  const acquired = await user.mutation(api.experience.acquire, {});
  expect(acquired.remainingSeconds).toBe(Math.max(0, remaining - 15));
});
