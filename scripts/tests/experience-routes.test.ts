...
import type { ExperienceService } from "../../src/components/app/experience-chrome";
...
vi.mock("@/components/app/experience-chrome", () => ({
...
beforeEach(() => {
  vi.stubEnv(
    "EXPERIENCE_ACCESS_SECRET",
    "test-only-experience-secret-at-least-32-characters",
  );
...
test("Mason sees X and can open it with a scoped relay grant", async () => {
  ...
  expect(page.props.services.find((service: ExperienceService) => service.id === "x")?.src).toBe(
    "https://experience.test/?u=https%3A%2F%2Fx.com%2F",
  );
  expect(page.props.accessToken).toEqual(expect.any(String));
  expect(
    (await AppPage({ params: Promise.resolve({ app: "x" }) })).props
      .initialAppId,
  ).toBe("x");
});
...
test("other users do not receive X in the catalogue, direct route or grant endpoint", async () => {
  ...
  expect(
    (await Catalogue()).props.services.some((service: ExperienceService) => service.id === "x"),
  ).toBe(false);
  await expect(
    AppPage({ params: Promise.resolve({ app: "x" }) }),
  ).rejects.toThrow();
  const response = await POST(
    new Request("https://app.test/experience/access", {
      method: "POST",
...
test("without the shared secret X is unavailable while existing apps still load", async () => {
  ...
  expect(
    page.props.services.find((service: ExperienceService) => service.id === "x")?.src,
  ).toBeNull();
  expect(
    page.props.services.find((service: ExperienceService) => service.id === "youtube")?.src,
