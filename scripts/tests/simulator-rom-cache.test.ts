...
import { accountKey, clearLocal, readLocal, readProgram, removeLocal, writeLocal, writeProgram } from "../../src/lib/simulator/local-store";
...
describe("local ROM cache", () => {
  it("upgrades existing save storage without losing progress", async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        tx.objectStore("progress").put(legacy, accountKey("legacy-owner") + "legacy");
  ...
  it("removes the ROM with its entry without affecting another account", async () => {
    ...
    await writeLocal("delete-owner", { contentHash: program.contentHash, mode: program.mode, label: "Test", revision: 0, updatedAt: 0, saves: {}, pending: {} });
    await removeLocal("delete-owner", program.contentHash);
    expect(await readProgram("delete-owner", program.contentHash)).toBeNull();
    expect(await readLocal("delete-owner", program.contentHash)).toBeUndefined();
    expect(await readProgram("keep-owner", program.contentHash)).toEqual(program);
  ...
  it("rejects bytes stored under the wrong content hash", async () => {
    const program = await identify(new Uint8Array(createTestProgram()).buffer);
    await writeProgram("corrupt-owner", { ...program, contentHash: "0".repeat(64) });
    await expect(readProgram("corrupt-owner", "0".repeat(64))).rejects.toThrow("damaged");
