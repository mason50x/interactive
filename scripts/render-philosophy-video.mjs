...
const canvasModule = await import(modulePath ? pathToFileURL(path.join(modulePath, 'index.js')).href : '@napi-rs/canvas');
...
const FLAT_THEME = process.argv.find(arg => arg.startsWith('--background='))?.split('=')[1];
if (FLAT_THEME && !['light', 'dark'].includes(FLAT_THEME)) throw new Error('Background must be light or dark');
...
async function render(compact, hands, phone) {
  ...
  const encoder = spawn('ffmpeg', [
    ...
    ...(!SMOOTH ? ['-map', '0:v', '-an', '-c:v', 'libvpx-vp9', '-pix_fmt', 'yuva420p', '-crf', '28', '-b:v', '0',
    '-row-mt', '1', '-threads', '4', '-cpu-used', '3', '-auto-alt-ref', '0', '-metadata:s:v:0', 'alpha_mode=1', path.join(OUTPUT, `${name}.webm`)] : []),
    ...(FLAT_THEME ? ['-map', '0:v', '-an', '-c:v', 'libx264', '-preset', 'veryfast', '-threads', '4', '-crf', '17', '-pix_fmt', 'yuv420p', '-g', '120', '-movflags', '+faststart', path.join(OUTPUT, `${name}.mp4`)] : [
    '-map', '0:v', '-an', '-c:v', 'hevc_videotoolbox', '-pix_fmt', 'bgra', '-alpha_quality', '0.85',
    '-allow_sw', '1', '-b:v', SMOOTH ? '14000k' : '4000k', '-tag:v', 'hvc1', '-movflags', '+faststart', path.join(OUTPUT, `${name}.mov`)]),
  ...
  for (let frame = 0; frame < DURATION * FPS; frame++) {
    ...
    const handY = [0, 1].map(side => (compact ? -40 : -80) + Math.sin(phase + side * 1.7) * 15);
    ...
    FINGERS.forEach(([fx, fy], i) => {
      ...
      ctx.lineWidth = 1.2;
      ctx.beginPath(); ctx.moveTo(anchorX, anchorY - 4); ctx.lineTo(x, y); ctx.stroke();
      ctx.save(); ctx.translate(x, y); ctx.rotate(-angle * 0.7);
      const phoneHeight = compact ? 89 : 100;
      ctx.drawImage(phone, 330, 0, 600, 1254, -phoneHeight * 0.24, 0, phoneHeight * 0.48, phoneHeight);
      ctx.restore();
    });
    for (let side = 0; side < 2; side++) {
      ctx.drawImage(hands, side * 768, 0, 768, 1024, handX[side], handY[side], handWidth, 1024 * imageScale);
    }
    ctx.restore();
    if (frame === 0 && !FLAT_THEME) await fs.writeFile(path.join(OUTPUT, `${name}-poster.png`), canvas.encodeSync('png'));
    ...
    if (frame % 90 === 0) console.log(`${name}: ${frame}/${DURATION * FPS} frames`);
  }
  encoder.stdin.end();
  const [code] = await finished;
  if (code !== 0) throw new Error(`FFmpeg exited ${code}`);
  console.log(`${name}: exported transparent ${SMOOTH ? 'HEVC' : 'WebM + HEVC'}, ${width}x${height}, ${FPS}fps, ${DURATION}s`);
}
