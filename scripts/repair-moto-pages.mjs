...
const catalogue = JSON.parse(await readFile(new URL("../src/lib/activities.catalogue.json", import.meta.url), "utf8"));
const games = catalogue.filter(({ slug }) => slugs.has(slug));
if (games.length !== slugs.size) throw new Error("Moto X3M catalogue entries missing.");
const dryRun = process.argv.includes("--dry-run");
const origin = process.env.ASSET_ORIGIN || process.env.NEXT_PUBLIC_ASSET_ORIGIN;
if (!origin) throw new Error("Set ASSET_ORIGIN to the live asset bucket origin.");
if (!dryRun) {
  for (const key of ["R2_ACCOUNT_ID", "R2_ACCESS_KEY_ID", "R2_SECRET_ACCESS_KEY", "R2_BUCKET"]) {
    if (!process.env[key]) throw new Error(`Missing ${key}; export the bucket credentials before uploading.`);
  }
...
try {
  ...
  for (const game of games) {
    const url = new URL(`activities/${game.path}/index.html`, `${origin.replace(/\/$/, "")}/`);
    ...
    if (!html.includes('src="motox3m.min.js"')) throw new Error(`${game.slug}: unexpected entry page`);
    const patched = patchGameHtml(html);
    assertPatched(game.slug, patched);
    await writeFile(join(directory, `${game.slug}.html`), patched);
    console.log(`${game.slug}: ${patched === html ? "already patched" : "repair validated"}`);
  }
  if (!dryRun) {
    const env = {
      ...process.env,
    ...
    for (const game of games) {
      const result = spawnSync("rclone", [
        "copyto", join(directory, `${game.slug}.html`),
        `R2:${process.env.R2_BUCKET}/activities/${game.path}/index.html`,
        "--ignore-times", "--header-upload", "Content-Type: text/html; charset=utf-8",
        "--header-upload", "Cache-Control: no-cache",
      ], { env, stdio: "inherit" });
      if (result.error) throw result.error;
      if (result.status !== 0) throw new Error(`Upload failed for ${game.slug}`);
    }
    console.log("Uploaded three entry pages. Purge their URLs from the Cloudflare cache, then reload the games.");
  } else {
